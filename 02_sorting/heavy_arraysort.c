...
void merge(struct HLoad list[], unsigned int left, unsigned int right) {
  unsigned int mid = (left + right) / 2;
  int i, j;

  struct HLoad *temp_left = (struct HLoad*)malloc(sizeof(struct HLoad) * (mid - left + 1));
  struct HLoad *temp_right = (struct HLoad*)malloc(sizeof(struct HLoad) * (right - mid));
  ...
  for (i = 0; i < mid - left + 1; i++) {
    ...
    copy_hload(temp_left + i, list + left + i);
  }
  // printf("\ntemp right : ");
  for (j = 0; j < right - mid; j++) {
    // temp_right[j] = list[mid + 1 + j];
    copy_hload(temp_right+ j, list + mid + 1 + j);
  }
  ...
  while (i < mid - left + 1 && j < right - mid) {
    if ((temp_left+i)->key <= (temp_right+j)->key) {
      // printf("%ld %ld\n", (temp_left+i)->key, (temp_right+j)->key);
      // list[index] = temp_left[i];
      copy_hload(list + index, temp_left + i);
      i++;
    } else {
      // printf("%ld %ld\n", (temp_left+i)->key, (temp_right+j)->key);
      // list[index] = temp_right[j];
      copy_hload(list + index, temp_right + j);
      j++;
    }
    // printf("%d ", list[index]);
    index++;
...
void merge_sort_heavy(struct HLoad list[], unsigned int left, unsigned int right) {
  if (left < right) {
    unsigned int mid = (left + right) / 2;
    // recursive merge_sort
    merge_sort_heavy(list, left, mid);
    merge_sort_heavy(list, mid + 1, right);
    // merge divided array
...
int main()
{
  ...
  if ( HA != NULL && numElements > 0 ) {
	...
	merge_sort_heavy(HA, 0, numElements-1);
	// free(HTmp);
	break;
      default:
	break;
